    ...
    <nav class="sidebar" style="width: 280px; background: linear-gradient(180deg, #1a1a2e 0%, #16213e 100%); min-height: 100vh; position: fixed; left: 0; top: 0; padding: 30px 20px; z-index: 1000;">
        ...
        <ul class="nav flex-column">
            <li class="nav-item mb-2">
                <a class="nav-link <?= $this->uri->segment(1) == 'admin' && !$this->uri->segment(2) ? 'active' : '' ?>" href="<?= base_url('admin') ?>" style="color: #a5b4fc; padding: 12px 15px; border-radius: 10px; display: flex; align-items: center; gap: 12px; transition: all 0.3s;">
                    <i class="bi bi-speedometer2" style="font-size: 18px;"></i>
                    <span>Dashboard</span>
                </a>
            </li>
            <li class="nav-item mb-2">
                <a class="nav-link <?= $this->uri->segment(1) == 'buku' ? 'active' : '' ?>" href="<?= site_url('buku') ?>" style="color: #a5b4fc; padding: 12px 15px; border-radius: 10px; display: flex; align-items: center; gap: 12px; transition: all 0.3s;">
                    <i class="bi bi-book" style="font-size: 18px;"></i>
                    <span>Kelola Buku</span>
                </a>
            </li>
            <li class="nav-item mb-2">
                <a class="nav-link <?= $this->uri->segment(1) == 'anggota' ? 'active' : '' ?>" href="<?= site_url('anggota') ?>" style="color: #a5b4fc; padding: 12px 15px; border-radius: 10px; display: flex; align-items: center; gap: 12px; transition: all 0.3s;">
                    <i class="bi bi-people" style="font-size: 18px;"></i>
                    <span>Kelola Anggota</span>
                </a>
            </li>
            <li class="nav-item mb-2">
                <a class="nav-link <?= $this->uri->segment(1) == 'transaksi' ? 'active' : '' ?>" href="<?= site_url('transaksi') ?>" style="color: #a5b4fc; padding: 12px 15px; border-radius: 10px; display: flex; align-items: center; gap: 12px; transition: all 0.3s;">
